...
void sauvegarde(){//sauvegarde l avancee (nombre de generations), la taille N de la matrice et la matrice.
    FILE * fichier;
    ...
    fwrite(mat, sizeof(case_t),N*N,fichier);//on recupere la matrice de la taille n*n

    fclose(fichier);
 }

void charger(){//charge la matrice de jeu, sa taille N et le nombre de generation (gen,n,mat)
    FILE * fichier;
